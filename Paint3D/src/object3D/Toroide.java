...
public class Toroide extends ObjectBase{

	//** Raggio spessore anello= minore;
	private double r;
	//** Raggio diametro anello= maggiore;
	private double R;
	double scalax=Math.PI;
	double scalay=2*Math.PI;
	public Toroide(int Id, double xxc,double yyc,double zzc,double r,double R,double alp,double bet,double gam,int red,int green,int blue,double reflexCoef,double rifrazCoef) {
	...
	@Override
	public boolean detect(/*double xxc,double yyc,double zzc,*/double xx1,double yy1,double zz1,double xx0,double yy0,double zz0,double xinc,double yinc,double zinc,double distmin0,PointResObj pointResIO,IdObj objId,int tipo){
		
		double dist=0,distmin=0,Rq=0/*,det=0*/;
		double r1m=0,r1p=0,r2m=0,r2p=0,r3m=0,r3p=0,r4m=0,r4p=0;
		...
		//** inizializazione centro oggetto 
	   	...
	   	R=this.getR();;//** raggio maggiore diametro anello
	   	r=this.getr();//** raggio minore spessore anello
	   	
	   	double alp=this.getAlp();
	   	double bet=this.getBet();
	   	double gam=this.getGam();
		
		double xi=pointResIO.getXi();
    	double yi=pointResIO.getYi();
    	double zi=pointResIO.getZi();
    	double Xr=pointResIO.getXr();
    	double Yr=pointResIO.getYr();
    	double Zr=pointResIO.getZr();
    	double Xg=pointResIO.getXg();
    	double Yg=pointResIO.getYg();
    	double Zg=pointResIO.getZg();
...
//		zzcI = xxc*a13+yyc*a23+zzc*a33;
		...
		*/
		...
		if(tern==1 || tern==3 || tern==5){// X,XY,XZ
			...
			av = a*a;
			bv = 2*a*b;
			cv = 2*a*c + b*b + 4*Rq*kzxqI;
			dv = 2*c*b + Rq*(kzxI*(zz0I-zzcI) - xx0I*kzxqI)*8;
			ev = c*c + Rq*kzxqI*xx0I*xx0I*4 + Rq*(zz0I-zzcI)*(zz0I-zzcI)*4 - Rq*kzxI*(zz0I-zzcI)*xx0I*8 - Rq*r*r*4;
			tern=1;
		}
			...
			av = a*a;
			bv = 2*a*b;
			cv = 2*a*c + b*b + 4*Rq*kzyqI;
			dv = 2*c*b + Rq*(kzyI*(zz0I-zzcI) - yy0I*kzyqI)*8;
			ev = c*c + Rq*kzyqI*yy0I*yy0I*4 + Rq*(zz0I-zzcI)*(zz0I-zzcI)*4 - Rq*kzyI*(zz0I-zzcI)*yy0I*8 - Rq*r*r*4;
			tern=2;
		}
		else{// Z
			kxzqI = kxzI*kxzI;
			...
			cv = 2*a*c + b*b + 4*Rq;
			dv = 2*c*b - Rq*zzcI*8;
			ev = c*c + Rq*zzcI*zzcI*4 - Rq*r*r*4;
			tern=4;
		}
		...
		//{
			...
			r1m=r1p=r2m=r2p=r3m=r3p=r4m=r4p=0;
			pointResTmp.setRv1m(r1m);
	    	pointResTmp.setRv1p(r1p);
	    	pointResTmp.setRv2m(r2m);
	    	pointResTmp.setRv2p(r2p);
	    	pointResTmp.setRv3m(r3m);
	    	pointResTmp.setRv3p(r3p);
	    	pointResTmp.setRv3m(r4m);
	    	pointResTmp.setRv3p(r4p);
	    	Utility.EquQuaNew(av,bv,cv,dv,ev,pointResTmp);
			r1m=pointResTmp.getRv1m();
			r1p=pointResTmp.getRv1p();
	    	r2m=pointResTmp.getRv2m();
	    	r2p=pointResTmp.getRv2p();
	    	r3m=pointResTmp.getRv3m();
	    	r3p=pointResTmp.getRv3p();
	    	r4m=pointResTmp.getRv4m();
	    	r4p=pointResTmp.getRv4p();
			...
			if(Math.abs(r3p)<Utility.EPS2  || Math.abs(Math.abs(r3p)-Math.PI)<(Utility.EPS)){
				// punto di intersezione retta di proiezione-solido in coord trasformate
				if(tern==1){
					xiI = r3m*Math.cos(r3p);
					yiI = yy0I + (xiI-xx0I)*kyxI;
					ziI = zz0I + (xiI-xx0I)*kzxI;
				}
				else if(tern==2){
					yiI = r3m*Math.cos(r3p);
					xiI = xx0I + (yiI-yy0I)*kxyI;
					ziI = zz0I + (yiI-yy0I)*kzyI;
				}
				else if(tern==4){
					ziI = r3m*Math.cos(r3p);
					xiI = xx0I + (ziI-zz0I)*kxzI;
					yiI = yy0I + (ziI-zz0I)*kyzI;
				}
				// punto di intersezione retta di proiezione-solido in coord originali
			...
			if(Math.abs(r4p)<Utility.EPS2 || Math.abs(Math.abs(r4p)-Math.PI)<(Utility.EPS)){
				// punto di intersezione retta di proiezione-solido in coord trasformate
				if(tern==1){
					xiI = r4m*Math.cos(r4p);
					yiI = yy0I + (xiI-xx0I)*kyxI;
					ziI = zz0I + (xiI-xx0I)*kzxI;
				}
				else if(tern==2){
					yiI = r4m*Math.cos(r4p);
					xiI = xx0I + (yiI-yy0I)*kxyI;
					ziI = zz0I + (yiI-yy0I)*kzyI;
				}
				else if(tern==4){
					ziI = r4m*Math.cos(r4p);
					xiI = xx0I + (ziI-zz0I)*kxzI;
					yiI = yy0I + (ziI-zz0I)*kyzI;
				}
				// punto di intersezione retta di proiezione-solido in coord originali
...
//				zcs = xcsI*a13I+ycsI*a23I+zcsI*a33I;
	...
	public void initMapping(){
		int Wi=(int)Math.ceil(Wscan);
		int Hi=(int)Math.ceil(Hscan);
	...
	public double[] getObjMapping(double x,double y,double z){
		...
		double[] vetOut;
	   	...
	   	double teta = Math.atan2((yI-yycI),(xI-xxcI));
		// centro sfera passante per il punto incidente in coord originali
	   	double xcsI = xxcI + R*Math.cos(teta);
	   	double ycsI = yycI + R*Math.sin(teta);
	   	double zcsI = zzcI;
	   	
	   	double xmt=0;
	   	if((yI - yycI)>=0)
			xmt = teta*R*(1/Math.PI);
		else if((yI - yycI)<0)
			xmt = (teta+2*Math.PI)*R*(1/Math.PI);
	   	
	   	double modulo_xy=Math.sqrt((xI - xcsI)*(xI - xcsI)+(yI - ycsI)*(yI - ycsI));
	   	if((xI - xcsI)*(xI-xxcI)+(yI - ycsI)*(yI-yycI)<0)
	   		modulo_xy = -modulo_xy;
	   	
	   	double ymt=0;
	   	if((zI - zcsI)>=0)
			ymt = (Math.atan2((zI - zcsI),modulo_xy))*r*(1/(2*Math.PI));
		else if((zI - zcsI)<0)
			ymt = (Math.atan2((zI - zcsI),modulo_xy)+2*Math.PI)*r*(1/(2*Math.PI));
		
	   	
	   	int xm = (int) Math.round(xmt*scalax);
	   	int ym = (int) Math.round(ymt*scalay);
	   	
//	   	if(xm<0 || ym<0 || xm>=(int)Wscan || ym>=(int)Hscan)
//	   		result[0]=0;
//	   	else{
...
//	   	}
	...
	public double[] map2Obj(int x,int y){
		...
		double[] vetOut;
	   	...
	   	vetIn[2] = Math.sin((y/scalay)*2*Math.PI/r)*r + zzcI;
	   	
	   	double modulus_xy = Math.cos((y/scalay)*2*Math.PI/r)*r;
	   	
	   	vetIn[0]=Math.cos((x/scalax)*Math.PI/R)*(R+modulus_xy) + xxcI;
	   	vetIn[1]=Math.sin((x/scalax)*Math.PI/R)*(R+modulus_xy) + yycI;
